...
const FloatingTerminal = () => {
  ...
  useEffect(() => {
    if (isTerminalPage) {
      const timer = setTimeout(() => setPageContainer(document.getElementById(TERMINAL_PAGE_CONTAINER_ID)), 50)
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.ctrlKey && e.key === '`') {
        ...
        setPipState(prev => {
          if (prev === 'minimized') { setHasBeenOpened(true); return isMobile() ? 'maximized' : 'pip' }
          return 'minimized'
        })
        if (btnHidden) setBtnHidden(false)
      }
      if (e.key === 'Escape' && !isTerminalPage && pipState !== 'minimized') setPipState('minimized')
    }
    window.addEventListener('keydown', handler)
    return () => window.removeEventListener('keydown', handler)
  }, [pipState, btnHidden, isTerminalPage])
  ...
  // === Page mode: portal into TerminalPage ===
  if (isTerminalPage && pageContainer) {
    return createPortal(
      <div className="flex flex-col h-full w-full overflow-hidden rounded-xl" style={{ background: '#0d1117' }}>
  ...
  // === Floating/PiP mode ===
  if (btnHidden && pipState === 'minimized') return null

  const isVisible = pipState === 'pip' || pipState === 'floating' || pipState === 'maximized'
  ...
  const defaultPipPos = { x: mobile ? 8 : window.innerWidth - pipSize.w - 16, y: mobile ? window.innerHeight - pipSize.h - 70 : window.innerHeight - pipSize.h - 24 }
  const currentPosition = isPip
  ...
  return (
    <>
      ...
      {/* PiP / Floating / Maximized */}
      {(isVisible || hasBeenOpened) && (
        <div
          ref={panelRef}
          className={`fixed z-[9990] flex flex-col overflow-hidden transition-all duration-200 ease-out ${isVisible ? 'opacity-100 pointer-events-auto scale-100' : 'opacity-0 pointer-events-none scale-[0.96]'}`}
          style={{
            ...(pipState === 'maximized'
              ? { inset: 0, borderRadius: 0 }
              : { left: currentPosition.x, top: currentPosition.y, width: currentSize.w, height: currentSize.h, borderRadius: isPip ? 14 : 18 }),
            background: '#0d1117',
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-0.5">
              ...
              {/* Minimizar: vira botão flutuante */}
              <button onClick={() => setPipState('minimized')} className="flex h-5 w-5 items-center justify-center rounded-md hover:bg-yellow-500/10 transition" title="Minimizar (botão)">
                <Minus size={9} className="text-gray-600 hover:text-yellow-400" />
              </button>
              {/* Fechar: esconde totalmente (Ctrl+` reabre) */}
              <button onClick={() => { setBtnHidden(true); setPipState('minimized') }} className="flex h-5 w-5 items-center justify-center rounded-md hover:bg-red-500/10 transition" title="Fechar (Ctrl+` reabre)">
